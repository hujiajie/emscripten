...
var PP_ENUM_LITERAL             = makePPEnum();
...
var PP_ENUM_SYNTHESIZE_LITERAL             = makePPEnum();
    ...
    try {
      ...
      while (ch1 = line[colStart]) {
        if (ch1 == ' ') {
          ...
          var int = line.substring(colStart, colEnd);
          if (parseInt(int).toString() != int) throw 'LexicalError';
          int = parseInt(int);
          tokens.push({
            type: PP_ENUM_INT,
            val: int,
      ...
      while (parseStack.length > 1) {
        ...
        case PP_ENUM_RELATIONAL_EXPR_OPT:
          if (tokens[lookahead].type == PP_ENUM_GT ||
            ...
            expansion = [
              { type: PP_ENUM_SYNTHESIZE_LITERAL, args: [], dstOffset: -1 },
              { type: PP_ENUM_LITERAL },
              { type: tokens[lookahead].type },
            ];
            parseStack[top - 1].args.push(tokens[lookahead].type);
            parseStack[top - 1].args.push(parseStack[top].id);
        ...
        case PP_ENUM_NOT_EXPR_EXT:
          ...
          [].push.apply(parseStack, expansion);
          break;
        case PP_ENUM_LITERAL:
          if (tokens[lookahead].type == PP_ENUM_INT ||
        ...
        case PP_ENUM_SYNTHESIZE_LITERAL:
          ...
          if (parseStack[top].args.length == 1) { // args = [ operand ]
            ...
            if (!(lhs.val in this)) {
              ...
              if (isFinite(this[lhs.val]) && Math.floor(this[lhs.val]) == this[lhs.val])
                ...
                lhs = null;
              rhs = rhs.type == PP_ENUM_INT ? rhs.val : null;
